...
> 本リポジトリは模擬案件（ポートフォリオ用・案件2）として作成しています。提案背景は [`requirements.md`](./requirements.md)（提案書）、実装レベルの詳細は [`docs/requirements-definition.md`](./docs/requirements-definition.md)（要件定義書・WordPress連携仕様を含む）、日々の運用手順は [`docs/operations-manual.md`](./docs/operations-manual.md)（運用手順書）を参照してください。
